...
class Utils {
	...
	fix_config(config) {
		if (config.system.debug === "enabled") {
			...
			config.site.url = `localhost:${config.server.express_port}`;
		}
		...
		for (let i = 0; i < config.site.pwa.files.length; ++i) {
		    if (config.site.pwa.files[i].includes(".min.css") || config.site.pwa.files[i].includes(".min.js")) {
				config.site.pwa.files[i] = `${config.site.pwa.files[i]}?v=${config.site.pwa.version}`;
		    }
		}
